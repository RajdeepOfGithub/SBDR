...
> **Note:** Tier 4 now exclusively handles Severe Genuine Distress cases routed to human relationship managers with care. Strategic Default cases are separated into Tier 5 via the B3.5 Audit Layer.

---

## 6b. Updated Classification — 5-Tier System with Strategic Default
...
flowchart LR
    ...
    T2["TIER 2\nSoft Assistance\n---\nHigh volatility + Neutral tone\n→ Skip a Payment"]
    T3["TIER 3\nHardship Restructuring\n---\nHigh distress + Declining spend\n→ 6-Month Plan"]
    T4["TIER 4\nSevere Genuine Distress\n---\nAnxious silence + Income disrupted\n→ Human Intervention + Care"]
    T5["TIER 5\nStrategic Default\n---\nStable income + Bust-out spend\n+ Deliberate silence\n→ B3.5 Audit Layer"]
    AUDIT["B3.5 Audit Layer\n---\nAll 5 conditions met?\nYes → Legal/Fraud\nNo → Downgrade to Tier 4"]
    ...
    style T3 fill:#e17055,color:#fff
    style T4 fill:#d63031,color:#fff
    style T5 fill:#2d3436,color:#fff
    style AUDIT fill:#d63031,color:#fff
```
...
## 7b. Strategic Default Detection — Behavioral Fingerprint

```mermaid
flowchart TB
    INPUT3["Tier 5\nCandidate"] --> SIG["Behavioral\nSignal Analysis"]

    SIG --> S1["Income Stability\nlc_annual_inc_mean stable\n6+ months pre-default"]
    SIG --> S2["Bust-Out Pattern\nSpending spike 40%+\n60 days before payment stop"]
    SIG --> S3["Selective Delinquency\nMissing unsecured debt\nCurrent on secured debt"]
    SIG --> S4["Communication Absence\nFinBERT score near 0.0\nDeliberate silence not anxiety"]
    SIG --> S5["Elevated Fraud Rate\nsp_fraud_rate above threshold\nSparkov signal"]

    S1 --> AUDIT3["B3.5 Audit Layer\nAll 5 conditions\nrequired"]
    S2 --> AUDIT3
    S3 --> AUDIT3
    S4 --> AUDIT3
    S5 --> AUDIT3
    ...
    AUDIT3 --> YES["All Met\n→ Legal/Fraud\nEscalation"]
    AUDIT3 --> NO["Any Failed\n→ Downgrade\nto Tier 4"]

    style INPUT3 fill:#2d3436,color:#fff
    style SIG fill:#2d3436,color:#fff
    style S1 fill:#636e72,color:#fff
    style S2 fill:#636e72,color:#fff
    style S3 fill:#636e72,color:#fff
    style S4 fill:#636e72,color:#fff
    style S5 fill:#636e72,color:#fff
    style AUDIT3 fill:#d63031,color:#fff
    style YES fill:#d63031,color:#fff
    style NO fill:#e17055,color:#fff
```

> **Design Principle:** The Audit Layer is rule-based — not ML — to ensure every fraud escalation is legally defensible. When in doubt, always downgrade to Tier 4.
...
## 9b. Strategic Default Risk Register

```mermaid
flowchart TB
    RISKS["Tier 5 Risk Register"]

    RISKS --> R1["Class Imbalance\n5-10% of population\nFix: SMOTE +\nXGBoost class_weight"]
    RISKS --> R2["False Positive\nWrongly accusing\ngenuine distress\nFix: Conservative threshold\nDefault to Tier 4"]
    RISKS --> R3["Feature Leakage\nPost-default info\nin training\nFix: Timestamp audit"]
    RISKS --> R4["Demographic Bias\nDisparate impact\nacross demographics\nFix: Fairness audit\non Tier 5 only"]
    RISKS --> R5["Explainability\nFraud needs legal\ndefensibility\nFix: SHAP + documented\naudit rules"]
    RISKS --> R6["Synthetic Limitation\nNo real fraud cases\nin training data\nFix: Deployment caveat\nin paper"]

    style RISKS fill:#d63031,color:#fff
    style R1 fill:#ff7675,color:#fff
    style R2 fill:#ff7675,color:#fff
    style R3 fill:#ff7675,color:#fff
    style R4 fill:#ff7675,color:#fff
    style R5 fill:#ff7675,color:#fff
    style R6 fill:#ff7675,color:#fff
```
...
> **Architecture Update:** The above diagram reflects the base pipeline. Tier 5 (Strategic Default) now sits between XGBoost output and the Legal escalation path, with the B3.5 Audit Layer as a mandatory checkpoint. See Section 6b for the updated tier flow.
...
| Dark Grey/Black | Strategic Default (Tier 5) / Bust-Out Detection |
